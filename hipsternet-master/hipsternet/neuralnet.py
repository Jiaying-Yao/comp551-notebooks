...
class NeuralNet(object):
    ...
    loss_funs = dict(
        ...
        cross_ent_noReg=loss_fun.cross_entropy_noReg,
    ...
    # loss functions without regularization
    dloss_funs = dict(
        cross_ent=loss_fun.dcross_entropy,
        cross_ent_noReg=loss_fun.dcross_entropy_noReg,
    ...
    def __init__(self, D, C, H, lam=1e-3, p_dropout=[.5,.8], loss='cross_ent', nonlin='relu', nlayer=3):
    ...
    def predict_proba(self, X):#not training , but predicting, so train=false
...
# use loss function without regularization
class FeedForwardNet(NeuralNet):

    def __init__(self, D, C, H, lam=1e-3, p_dropout=[.5,.8], loss='cross_ent', nonlin='relu', nlayer = 3):
        super().__init__(D, C, H, lam, p_dropout, loss, nonlin, nlayer)
    ...
    def backward(self, y_pred, y_train, cache):
        if self.nlayer == 2:
            X, h1_cache, score_cache, nl_cache1, u1, bn1_cache = cache
            
            # Output layer
            grad_y = self.dloss_funs[self.loss](y_pred, y_train)
            
            # Second layer
            dh1, dW4, db4 = l.fc_backward(grad_y, score_cache)
            dW4 += reg.dl2_reg(self.model['W4'], self.lam)
            ...
            grad = dict(
                W1=dW1, W4=dW4, b1=db1, b4=db4, gamma1=dgamma1, beta1=dbeta1
            )
        ...
        if self.nlayer == 3:
            ...
            # Third layer
            dh2, dW4, db4 = l.fc_backward(grad_y, score_cache)
            dW4 += reg.dl2_reg(self.model['W4'], self.lam)
            ...
            grad = dict(
                W1=dW1, W2=dW2, W4=dW4, b1=db1, b2=db2, b4=db4, gamma1=dgamma1,
                gamma2=dgamma2, beta1=dbeta1, beta2=dbeta2
            )
        
        if self.nlayer == 4:
            X, h1_cache, h2_cache, h3_cache, score_cache, nl_cache1, nl_cache2, nl_cache3, u1, u2, u3, bn1_cache, bn2_cache, bn3_cache = cache
            ...
            #Third layer
            dh2, dW3, db3 = l.fc_backward(dh3, h3_cache)
            ...
            grad = dict(
                W1=dW1, W2=dW2, W3=dW3, W4=dW4, b1=db1, b2=db2, b3=db3, b4=db4, gamma1=dgamma1,
                gamma2=dgamma2, gamma3=dgamma3, beta1=dbeta1, beta2=dbeta2, beta3=dbeta3
            )
    ...
    def _init_model(self, D, C, H):
        self.model = dict(
            ...
            W3=np.random.randn(H, H) / np.sqrt(H / 2.),
...
class ConvNet(NeuralNet):
    ...
    def __init__(self, D, C, H, lam=1e-3, p_dropout=[0.5], loss='cross_ent', nonlin='relu'):
