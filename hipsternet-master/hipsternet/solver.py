...
def sgd(nn, X_train, y_train, val_set=None, alpha=1e-3, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
    ...
    for iter in range(1, n_iter + 1):
        ...
        for layer in grad:
            nn.model[layer] -= alpha * grad[layer]
            if max_norm != None:
                nn.model[layer] = reg.limit_norm(nn.model[layer], max_val=max_norm)
...
def momentum(nn, X_train, y_train, val_set=None, alpha=1e-3, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
...
def nesterov(nn, X_train, y_train, val_set=None, alpha=1e-3, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
    velocity = {k: np.zeros_like(v) for k, v in nn.model.items()}
    gamma = .9
...
def adagrad(nn, X_train, y_train, val_set=None, alpha=1e-3, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
...
def rmsprop(nn, X_train, y_train, val_set=None, alpha=1e-3, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
    cache = {k: np.zeros_like(v) for k, v in nn.model.items()}
    gamma = .9
...
def adam(nn, X_train, y_train, val_set=None, alpha=0.001, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
...
def adam_rnn(nn, X_train, y_train, alpha=0.001, mb_size=256, n_iter=2000, print_after=100, max_norm = None):
    M = {k: np.zeros_like(v) for k, v in nn.model.items()}
    R = {k: np.zeros_like(v) for k, v in nn.model.items()}
    beta1 = .9
    beta2 = .999
